...
The example in Figure \ref{fig:codeowners_settings} shows the pull request reviews settings. The subsettings of \uv{\textit{Require pull request reviews before merging}} may not be set without \uv{\textit{Required approving reviews}} which minimal value is one. The developers would like to have this helpful enhancement of the development provided by the GitHub. Solution how to solve this problem resulted in a design which will enhance the bot - addition of a new feature to the bot which will keep the pull request mergeable.
