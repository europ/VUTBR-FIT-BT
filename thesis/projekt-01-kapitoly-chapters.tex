...
% TODO REMOVE
\begin{comment}
Command `make normostrany` fails here. The code above contains $ which stops the `detex -n FILE` at this point and the text behind the $ is ignored until the end
\end{comment}
...
\subsection{Enhancements {\color{red}\textbf{TODO asi zly nazov}}}
...
The ManageIQ bot does not support a commands allowing project members to request for a pull request review or to remove a request for a pull request review. An issue\footnote{\url{https://github.com/ManageIQ/miq_bot/issues/337}} has been created describing the bot's restriction. The commands were named based on the suggestion from the developers and discussion in the issue mentioned before.

\subsection{Issues}
{\color{red}\textbf{TODO}}: octokit version problem (require to update), octokit missing method (required to implement), octokit VCR cassette (solved), fixing pull request (yolo merge - missing dependence)


\subsection{Issues}

{\color{red}\textbf{TODO}}\\
Related pull request (ADD reviewer): \url{https://github.com/ManageIQ/miq_bot/pull/408}.
