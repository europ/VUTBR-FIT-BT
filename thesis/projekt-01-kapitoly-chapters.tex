...
In these days, Continuous Integration (CI) is more often used in larger projects, where are multiple developers working on one and the same software product. This process ensure fast software developement, called eXtreme Programming (XP), known as agile software developement methodology. The methodology is mainly used for the speed of the developement, nevertheless many disruptions may be fetched into the developing software. Nowadays, although this type of software developement has many disadvantages, it is still much more often used on larger projects. You may also know that the continuous integration is a part of the following open-source projects e.g. facebook, twitter, mozilla. These projects are using one of many famous continuous integration service Travis CI. Excluding Travis CI, there are plenty of other continuous integration services you may heard about, such as Jenkins, TeamCity, CircleCI, GitLab CI, Codeship and so on.\newline
\newline
The developement process requires many code checking tools after every single code change in the source code of the software. Forasmuch as with every signle change of code, there is a possibility to add, fix, derange or deteriorate any parts of the software product. These tools provide an automated code review and they afford a quick feedback by which they try to prevent these code impairments. Feedback is sent to the developer, who has made the change in the code, about his adjustment. The automated process which provides the code review do not bother without executing a huge amount of tests. Above mentioned process is conducted via continuous integration server, which compile the code, runs scripts and test. The results are aggregated and feedback is given to the developer who has made this code change. Continuous integration server is invoked every single time after any change is fetched in the source code and he had to execute the stated acts which are predefined. In next chapters we describe in details how does this workflow works and what steps are required to run.\newline
\newline
This thesis attempts to explain how the fundamentals of continuous integration and automated code review work. It describes how it is integrated to the 
software developement, and how it works on an extensive project nowadays. Examples will be based on open-source project e.g. ManageIQ, which is a cloud manager 
founded by RedHat. The developing process of the ManageIQ rests in agile and stable progress of developement. These main factors of the developement process 
could not be reached without a quick feedback to the developers working on project about their changes, that are submitted to the software product.
